...
// Let's add a special mock function for testing that middleware is activated
const mockedAuthMiddleWare = (passport as any)
	.authenticateMiddleware as jest.MockedFunction<(...args: any[]) => any>
const mockedAuthenticate = passport.authenticate as jest.MockedFunction<
...
describe("context.authenticate", () => {
	...
	test("rejects when passport returns error", async () => {
		...
		mockedAuthenticate.mockImplementationOnce((name, options, done) =>
			done(expectedError)
		)
		const context = buildContext({ req: {}, res: {} } as ContextParams)
