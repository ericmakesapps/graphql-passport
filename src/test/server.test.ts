...
describe("Test fullscale server implementation", () => {
  ...
  `
		...
		expect(ret).toHaveProperty(
			...
			expect.arrayContaining([
				expect.objectContaining({
					message: expect.stringContaining("failed to authenticate user")
				})
			])
		)
    ...
    `
		...
		expect(ret).toHaveProperty(
			"errors.0.message",
			"You need to be logged in to access launch queries"
		)
    ...
    `
		...
		expect(ret).toHaveProperty(
			"errors.0.message",
			"You need to be logged in to access launch mutations"
		)
		...
		expect(ret).toHaveProperty(
			"errors.0.message",
			"You have insufficient privileges for launch mutations"
		)
